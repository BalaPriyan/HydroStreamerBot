...
class Database:
    def __init__(self, uri, database_name):
        ...
        self.tokens_collection = self.db['tokens']  # Collection for storing tokens
        self.verification_collection = self.db['verification']
    ...
    async def get_token(self, id, token):
        user = self.tokens_collection.find_one({'id': id, 'token': token})
        if user:
            return user['used']
        return None

    async def set_token(self, id, token, used=False):
    ...
    async def set_verification_time(self, id, verification_time):
        self.verification_collection.update_one(
            {'id': id},
            {'$set': {'verification_time': verification_time}},
            upsert=True
        )

    async def get_verification_time(self, id):
        user = self.verification_collection.find_one({'id': id})
        if user and 'verification_time' in user:
            return user['verification_time']
        return None

    async def is_verification_expired(self, id):
        verification_time = await self.get_verification_time(id)
        if verification_time is None:
            return True
        return datetime.now(pytz.utc) - verification_time > timedelta(hours=24)
